...
@section('content')
    ...
    @php
        ...
        function drMandatExpire($copro): bool
        {
            ...
            // Si un nom/SIREN/SIRET est connu → mandat actif, pas expiré
            $hasActiveRep =
                !empty(dr_value($copro, ['representant_legal_nom', 'syndic_nom'], null)) ||
                !empty(dr_value($copro, ['siren_syndic', 'siren_representant_legal'], null)) ||
                !empty(dr_value($copro, ['siret_syndic', 'siret_representant_legal'], null));
            if ($hasActiveRep) {
                return false;
            }
            // Date de fin = il y avait un syndic → gestionnaire de fait
            return !empty(drDateFin($copro));
        }
        ...
        function drRepConnu($copro): bool
        {
            ...
            if (
                !empty(
                    dr_value(
                        $copro,
                        ['representant_legal_nom', 'syndic_nom', 'raison_sociale_representant_legal'],
                        null,
                    )
                )
            ) {
            ...
            if (!empty(dr_value($copro, ['siren_syndic', 'siren_representant_legal'], null))) {
                return true;
            }
            if (!empty(dr_value($copro, ['siret_syndic', 'siret_representant_legal'], null))) {
                return true;
            }
        ...
        $coproPrincipale =
            $coprosAvecRep->sortByDesc(fn($c) => (int) dr_value($c, ['score_match'], 0))->first() ??
            ($copros
        ...
        $sirenSyndic = $coproPrincipale
            ? dr_value($coproPrincipale, ['siren_syndic', 'siren_representant_legal'], null)
            : null;
        ...
        if (!$representantConnu && $copros->isNotEmpty()) {
            ...
            if ($anyRep) {
                ...
                if (!$representantNom) {
                    $representantNom = dr_value($anyRep, ['representant_legal_nom', 'syndic_nom'], null);
                }
                if (!$sirenSyndic) {
                    $sirenSyndic = dr_value($anyRep, ['siren_syndic', 'siren_representant_legal'], null);
                }
                if (!$mandatExpire) {
                    $mandatExpire = drMandatExpire($anyRep);
                }
                if (!$dateFinMandat) {
                    $dateFinMandat = drDateFin($anyRep);
                }
            }
        }
        ...
        $syndicsAffiches = $syndicsAffiches
            ->filter()
            ->unique(
                fn($s) => dr_value($s, ['siret'], null) ?:
                dr_value($s, ['siren'], null) ?:
                dr_value($s, ['nom'], uniqid()),
            )
            ->values();
            ...
            <div class="dr-stats">
                <div class="dr-stat-card" data-tab="eligibilite">
                    ...
                    <div class="dr-stat-value {{ $hasAnyZone ? 'danger' : 'success' }}">
                        {{ $hasAnyZone ? 'À exclure' : 'Exploitable' }}</div>
                </div>
            ...
            @else
                ...
                <div class="dr-panel active" id="panel-eligibilite">
                    ...
                    @if ($qpv)
                        <div class="dr-grid">
                            <div class="dr-field">
                                ...
                                <div class="dr-field-value"
                                    style="color:{{ $hasQpv2024 ? 'var(--dr-danger)' : 'var(--dr-success)' }}"><i
                                        class="fa-solid {{ $hasQpv2024 ? 'fa-circle-xmark' : 'fa-circle-check' }}"></i>
                                    {{ $hasQpv2024 ? 'Zone détectée' : 'Hors zone' }}</div>
                            </div>
                            <div class="dr-field">
                                <div class="dr-field-label">QP 2015</div>
                                <div class="dr-field-value"
                                    style="color:{{ $hasQpv2015 ? 'var(--dr-danger)' : 'var(--dr-success)' }}"><i
                                        class="fa-solid {{ $hasQpv2015 ? 'fa-circle-xmark' : 'fa-circle-check' }}"></i>
                                    {{ $hasQpv2015 ? 'Zone détectée' : 'Hors zone' }}</div>
                            </div>
                            <div class="dr-field">
                                <div class="dr-field-label">ZFU</div>
                                <div class="dr-field-value"
                                    style="color:{{ $hasZfu ? 'var(--dr-danger)' : 'var(--dr-success)' }}"><i
                                        class="fa-solid {{ $hasZfu ? 'fa-circle-xmark' : 'fa-circle-check' }}"></i>
                                    {{ $hasZfu ? 'Zone détectée' : 'Hors zone' }}</div>
                            </div>
                            <div class="dr-field">
                        ...
                        @foreach ($qpv['checks'] ?? [] as $index => $check)
                            ...
                            <div class="dr-record">
                                <div class="dr-record-header">
                                    ...
                                    <div class="dr-status {{ $hasZone ? 'danger' : 'success' }}"><i
                                            class="fa-solid {{ $hasZone ? 'fa-location-crosshairs' : 'fa-circle-check' }}"></i>
                                        {{ $hasZone ? 'Zone détectée' : 'Hors zone' }}</div>
                                </div>
                                <div class="dr-grid">
                                    <div class="dr-field">
                                        <div class="dr-field-label">Adresse BAN</div>
                                        <div class="dr-field-value">{{ $cand['adresse'] ?? '-' }}</div>
                                    </div>
                                    <div class="dr-field">
                                        <div class="dr-field-label">Score BAN</div>
                                        <div class="dr-field-value">{{ $cand['score'] ?? '-' }}</div>
                                    </div>
                                    <div class="dr-field">
                                        <div class="dr-field-label">Coordonnées</div>
                                        <div class="dr-field-value">{{ $cand['latitude'] ?? '-' }},
                                            {{ $cand['longitude'] ?? '-' }}</div>
                                    </div>
                                    <div class="dr-field">
                                        <div class="dr-field-label">Zone</div>
                                        <div class="dr-field-value">
                                            @if ($hasZone)
                                                @foreach ($res['matches'] ?? [] as $type => $match)
                                                    @if ($match['found'] ?? false)
                                                        {{ strtoupper($type) }} :
                                                        {{ $match['nom'] ?? ($match['code'] ?? 'Zone') }}<br>
                                                    @endif
                                                @endforeach
                                            @else
                                                <span style="color:var(--dr-success)">Aucune zone</span>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                        ...
                        @endforeach
                    @else
                        <div class="dr-empty">Aucun contrôle QPV/ZFU disponible.</div>
                    @endif
                </div>
                ...
                <div class="dr-panel" id="panel-representant">
                    <div class="dr-panel-header">
                        ...
                        <div style="display:flex;flex-direction:column;align-items:flex-end;gap:8px;">
                            {{-- Badge principal --}}
                            @if ($representantConnu)
                                <div class="dr-status success"><i class="fa-solid fa-circle-check"></i> Avec représentant
                                    légal</div>
                            @else
                    ...
                    @else
                        ...
                        @if ($mandatExpire)
                            <div
                                style="background:#fff7ed;border:1.5px solid #f97316;border-radius:16px;padding:14px 18px;margin-bottom:20px;display:flex;align-items:flex-start;gap:12px;">
                        ...
                        @if ($hasMultipleImmat)
                            <div
                                style="background:#fff7ed;border:1.5px solid #f59e0b;border-radius:16px;padding:14px 18px;margin-bottom:20px;display:flex;align-items:flex-start;gap:12px;">
                                ...
                                <div>
                                    ...
                                    <div style="font-size:.82rem;color:#b45309;line-height:1.5;">
                                        {{ $nbImmatriculations }} copropriétés immatriculées trouvées. Consultez l'onglet
                                        <strong>Copropriétés</strong> pour voir toutes les immatriculations.
                                    </div>
                                </div>
                            </div>
                        @endif
                        ...
                        <div class="dr-grid">
                            ...
                            <div class="dr-field copyable"
                                ...
                                <div class="dr-field-value">
                                    {{ dr_value($coproPrincipale, ['representant_legal_type', 'type_syndic']) }}</div>
                            </div>
                            ...
                            <div class="dr-field">
                                ...
                                <div class="dr-field-value">{{ dr_value($coproPrincipale, ['nombre_lots_habitation']) }}
                                </div>
                            </div>
                            <div class="dr-field">
                            ...
                            @if ($mandatExpire)
                                <div class="dr-field">
                                    ...
                                    <div class="dr-field-value" style="color:var(--dr-warning);"><i
                                            class="fa-solid fa-clock"></i> {{ $dateFinMandat }}</div>
                                </div>
                            @endif
                        </div>
                        ...
                        @if ($hasMultipleImmat)
                            <h3 style="margin:24px 0 12px;font-size:.95rem;font-weight:800;"><i
                                    class="fa-solid fa-list-check" style="color:var(--dr-blue);"></i> Toutes les
                                immatriculations ({{ $nbImmatriculations }})</h3>
                                ...
                                <div
                                    ...
                                    <div style="flex:1;min-width:0;">
                                        ...
                                        <div style="font-size:.75rem;color:var(--dr-muted);margin-top:2px;">
                                            ...
                                            @if (!empty(dr_value($c, ['representant_legal_nom', 'syndic_nom'], null)))
                                                — {{ dr_value($c, ['representant_legal_nom', 'syndic_nom']) }}
                                            @endif
                                            @if (!empty(dr_value($c, ['siren_syndic', 'siren_representant_legal'], null)))
                                                — SIREN : {{ dr_value($c, ['siren_syndic', 'siren_representant_legal']) }}
                                            @endif
                                            @if ($cMandatExp)
                                                — <span style="color:var(--dr-warning);">Mandat expiré le
                                                    {{ drDateFin($c) }}</span>
                                            @endif
                                            @if (!$cHasRep)
                                    ...
                                    <div
                                        style="font-size:.75rem;font-weight:800;color:{{ $cHasRep ? 'var(--dr-success)' : 'var(--dr-muted)' }}">
                                        Score {{ dr_value($c, ['score_match'], '-') }}</div>
                                </div>
                            @endforeach
                        @endif
                    @endif
                    ...
                    <div class="dr-panel-header">
                        <div class="dr-panel-title">
                            ...
                            <div>
                                ...
                                <p>Identifiant unique du bâtiment</p>
                            </div>
                        </div>
                        @if ($rnbId)
                            <div class="dr-status success"><i class="fa-solid fa-check-circle"></i>
                                {{ $rnbAddressesCount }} adresse(s)</div>
                        @endif
                    </div>
                    @if ($rnbId)
                        <div class="dr-grid">
                            <div class="dr-field copyable" data-copy="{{ $rnbId }}">
                                ...
                                <div class="dr-field-value"><code style="font-size:1rem;">{{ $rnbId }}</code>
                                </div>
                            </div>
                            <div class="dr-field">
                                <div class="dr-field-label">Statut</div>
                                <div class="dr-field-value"><span class="dr-status success"
                                        style="display:inline-flex;"><i class="fa-solid fa-check-circle"></i>
                                        Construit</span></div>
                            </div>
                            <div class="dr-field">
                                <div class="dr-field-label">Adresses associées</div>
                                <div class="dr-field-value">{{ $rnbAddressesCount }}</div>
                            </div>
                        </div>
                        @if ($rnbAddressesCount > 0)
                            <h3 style="margin:28px 0 16px;font-size:1rem;font-weight:800;"><i
                                    class="fa-solid fa-list"></i> Adresses liées</h3>
                            <div class="dr-grid">
                                    ...
                                    <div class="dr-record" style="margin-top:0;">
                                        ...
                                        @if (!empty($addrInfo['cle_ban']))
                                            <div class="dr-field copyable" data-copy="{{ $addrInfo['cle_ban'] }}">
                                                <div class="dr-field-label">Clé BAN <i class="fa-regular fa-copy"></i>
                                                </div>
                        ...
                        @endif
                    @else
                        <div class="dr-empty"><i class="fa-solid fa-diagram-project"></i> Aucun identifiant RNB trouvé.
                        </div>
                    @endif
                </div>
                    ...
                    <div class="dr-panel-header">
                        <div class="dr-panel-title">
                            ...
                            <div>
                                ...
                                <p>Parcelles cadastrales détectées</p>
                            </div>
                        </div>
                    </div>
                    @forelse(($resultat['cadastre']??[]) as $parcelle)
                        <div class="dr-record">
                            ...
                            <div class="dr-grid">
                                ...
                                <div class="dr-field">
                                    <div class="dr-field-label">Commune</div>
                                    <div class="dr-field-value">{{ $parcelle['commune'] ?? '-' }}</div>
                                </div>
                                <div class="dr-field">
                                    <div class="dr-field-label">Section / Numéro</div>
                    ...
                    @forelse(($resultat['batiments']??[]) as $batiment)
                        <div class="dr-record">
                            ...
                            <div class="dr-grid">
                                ...
                                <div class="dr-field">
                                    <div class="dr-field-label">Nb logements</div>
                                    <div class="dr-field-value">{{ dr_value($batiment, ['nombre_logements']) }}</div>
                                </div>
                                <div class="dr-field">
                                    <div class="dr-field-label">Nb niveaux</div>
                                ...
                                <div class="dr-field">
                                    <div class="dr-field-label">Type chauffage</div>
                                    <div class="dr-field-value">
                                        {{ dr_value($batiment, ['type_chauffage', 'chauffage_principal', 'type_installation_chauffage']) }}
                                    </div>
                                </div>
                                <div class="dr-field">
                                    <div class="dr-field-label">Énergie chauffage</div>
                                    <div class="dr-field-value">
                    ...
                    <div class="dr-panel-header">
                        <div class="dr-panel-title">
                            ...
                            <div>
                                ...
                                <p>Dénominations et SIREN détectés via BDNB — enrichis RNE local</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="dr-record">
                            <div class="dr-record-header">
                                <div class="dr-record-title">
                                    {{ $proprietaire['nom_bdnb'] ?? ($proprietaire['nom'] ?? 'Propriétaire BDNB') }}</div>
                                <div class="dr-status success"><i class="fa-solid fa-building-circle-check"></i>
                                    Entreprise</div>
                            </div>
                            <div class="dr-grid">
                                <div class="dr-field copyable"
                                    ...
                                    <div class="dr-field-value">
                                        {{ $proprietaire['nom_bdnb'] ?? ($proprietaire['nom'] ?? '-') }}</div>
                                </div>
                                ...
                                <div class="dr-field">
                                    ...
                                    <div class="dr-field-value">
                                        ...
                                        @if ($src === 'rne_local')
                                            <span class="dr-status success" style="display:inline-flex;font-size:11px;"><i
                                                    class="fa-solid fa-database"></i> RNE local</span>
                                        @elseif($src === 'bdnb_only')
                                            <span class="dr-status warning" style="display:inline-flex;font-size:11px;"><i
                            ...
                            @if (!empty($proprietaire['url_pappers']))
                                <div style="margin-top:16px;"><a href="{{ $proprietaire['url_pappers'] }}"
                                        target="_blank" class="dr-btn dr-btn-primary"><i
                                            class="fa-solid fa-arrow-up-right-from-square"></i> Voir la fiche</a></div>
                            @endif
                        </div>
                    @empty
                ...
                <div class="dr-panel" id="panel-coproprietes">
                    <div class="dr-panel-header">
                        ...
                        @if ($hasMultipleImmat)
                            <div class="badge-multi-immat"><i class="fa-solid fa-triangle-exclamation"></i>
                                {{ $nbImmatriculations }} immatriculations</div>
                        @endif
                    </div>
                    @forelse(($resultat['coproprietes']??[]) as $copro)
                        ...
                        <div class="dr-record">
                            <div class="dr-record-header">
                                <div class="dr-record-title">
                                    {{ dr_value($copro, ['nom_copropriete', 'nom_usage_copropriete']) }}</div>
                                <div style="display:flex;flex-direction:column;align-items:flex-end;gap:6px;">
                                    <div class="dr-status {{ $cRepConnu ? 'success' : 'warning' }}">
                                        <i class="fa-solid {{ $cRepConnu ? 'fa-circle-check' : 'fa-circle-info' }}"></i>
                                        Score {{ dr_value($copro, ['score_match'], '-') }}
                                    </div>
                                    @if ($cMandatExp)
                                        <div class="badge-mandat-expire"><i class="fa-solid fa-clock"></i> Mandat expiré
                                        </div>
                                    @endif
                                </div>
                            </div>
                            <div class="dr-grid">
                                <div class="dr-field copyable"
                                    data-copy="{{ dr_value($copro, ['adresse_complete', 'adresse_reference']) }}">
                                    <div class="dr-field-label">Adresse RNIC <i class="fa-regular fa-copy"></i></div>
                                    <div class="dr-field-value">
                                ...
                                <div class="dr-field">
                                    ...
                                    <div class="dr-field-value">
                                        @if ($cRepConnu)
                                            ...
                                            @if ($cMandatExp)
                                                <span
                                                    style="display:block;font-size:.7rem;color:var(--dr-warning);margin-top:3px;">⏱
                                                    Mandat expiré le {{ $cDateFin }}</span>
                                            @endif
                                        @else
                                ...
                                <div class="dr-field copyable"
                                    data-copy="{{ dr_value($copro, ['siren_syndic', 'siren_representant_legal']) }}">
                                    <div class="dr-field-label">SIREN syndic <i class="fa-regular fa-copy"></i></div>
                                    <div class="dr-field-value">
                                        <code>{{ dr_value($copro, ['siren_syndic', 'siren_representant_legal']) }}</code>
                                    </div>
                                </div>
                                <div class="dr-field copyable"
                                    data-copy="{{ dr_value($copro, ['siret_syndic', 'siret_representant_legal']) }}">
                                    <div class="dr-field-label">SIRET syndic <i class="fa-regular fa-copy"></i></div>
                                    <div class="dr-field-value">
                                        <code>{{ dr_value($copro, ['siret_syndic', 'siret_representant_legal']) }}</code>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="dr-panel-header">
                        <div class="dr-panel-title">
                            ...
                            <div>
                                ...
                                <p>Informations INPI RNE des syndics et représentants légaux</p>
                            </div>
                        </div>
                    </div>
                    @forelse($syndicsAffiches as $syndic)
                        <div class="dr-record">
                            ...
                            @if (dr_value($syndic, ['url_pappers'], null) !== '-')
                                <div style="margin-top:16px;"><a href="{{ dr_value($syndic, ['url_pappers']) }}"
                                        target="_blank" class="dr-btn dr-btn-primary"><i
                                            class="fa-solid fa-arrow-up-right-from-square"></i> Voir la fiche</a></div>
                            @endif
                            <details>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            statCards.forEach(card => card.addEventListener('click', function() {
