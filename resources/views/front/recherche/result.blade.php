...
@section('content')
    ...
    <style>
        ...
        .dr-stat-card {
            ...
            border-radius: 20px; padding: 18px; transition: all .2s; cursor: pointer;
        }
        .dr-stat-card:hover { transform: translateY(-2px); box-shadow: 0 12px 24px rgba(0,0,0,.06); }
        .dr-stat-icon {
            width: 44px; height: 44px; background: #e6f0ff; border-radius: 14px;
            display: flex; align-items: center; justify-content: center;
            color: var(--dr-blue); font-size: 1.2rem; margin-bottom: 12px;
        }
        ...
        .dr-panel-header {
            ...
            padding-bottom: 20px; border-bottom: 1px solid var(--dr-border);
        }
        .dr-panel-title { display: flex; gap: 16px; align-items: flex-start; }
        .dr-panel-icon {
            width: 52px; height: 52px; background: #e6f0ff; border-radius: 18px;
            display: flex; align-items: center; justify-content: center;
            color: var(--dr-blue); font-size: 1.3rem; flex-shrink: 0;
        }
        ...
        .dr-field {
            ...
            border-radius: 16px; padding: 14px; transition: all .2s; cursor: pointer;
        }
        .dr-field:hover { background: #f1f5f9; border-color: var(--dr-blue); }
        .dr-field-label {
            font-size: .65rem; text-transform: uppercase; letter-spacing: .5px;
            color: var(--dr-muted); font-weight: 800; margin-bottom: 6px;
                ...
                <div class="dr-panel-header">
                    ...
                    @if ($rnbId)
                        <div class="dr-status success">
                            <i class="fa-solid fa-check-circle"></i> {{ $rnbAddressesCount }} adresse(s) associée(s)
                        </div>
                    @endif
                </div>
                <div class="dr-panel-body">
                    @if ($rnbId)
                        ...
                        <div class="info-box">
                            ...
                            <div>
                                ...
                                <a href="#" target="_blank">Envoyez votre signalement</a> — tout le monde peut apporter sa pierre au RNB.
                            </div>
                        </div>
                    @else
                        <div class="dr-empty">
                ...
                <div class="dr-panel-header">
                    <div class="dr-panel-title">
                        ...
                        <div>
                            ...
                            <p>Dénominations et SIREN détectés dans les données BDNB — enrichis via RNE local</p>
                        </div>
                    </div>
                </div>
                <div class="dr-panel-body">
                    @forelse (($resultat['proprietaires_bdnb'] ?? []) as $proprietaire)
                        <div class="dr-record">
                            <div class="dr-record-header">
                                ...
                                <div class="dr-status success">
                                    <i class="fa-solid fa-building-circle-check"></i> Entreprise
                                </div>
                            </div>
                            <div class="dr-grid">
                                <div class="dr-field copyable" data-copy="{{ $proprietaire['nom_bdnb'] ?? $proprietaire['nom'] ?? '-' }}">
                                    <div class="dr-field-label">Dénomination BDNB <i class="fa-regular fa-copy"></i></div>
                                    <div class="dr-field-value">{{ $proprietaire['nom_bdnb'] ?? $proprietaire['nom'] ?? '-' }}</div>
                                </div>
                                    ...
                                    <div class="dr-field-value">
                                        ...
                                        @elseif ($src === 'bdnb_only')
                                            <span class="dr-status warning" style="display:inline-flex;font-size:11px;">
                                                <i class="fa-solid fa-building"></i> BDNB uniquement
                                            </span>
                                        @else
                                            <span class="dr-status warning" style="display:inline-flex;font-size:11px;">
                                                <i class="fa-solid fa-circle-info"></i> {{ $src ?: 'Non enrichi' }}
                                            </span>
                                        @endif
                                    </div>
                                </div>
                        ...
                        <div class="dr-record">
                            <div class="dr-record-header">
                                ...
                                <div class="dr-status {{ $coproRepConnu ? 'success' : 'warning' }}">
                                    <i class="fa-solid {{ $coproRepConnu ? 'fa-circle-check' : 'fa-circle-info' }}"></i>
                            ...
                            <div class="dr-grid">
                                <div class="dr-field copyable" data-copy="{{ dr_value($copro, ['adresse_complete']) }}">
                                    <div class="dr-field-label">Adresse RNIC <i class="fa-regular fa-copy"></i></div>
                                    <div class="dr-field-value">
                                        {{ dr_value($copro, ['adresse_complete']) }}<br>
                                        {{ dr_value($copro, ['code_postal']) }} {{ dr_value($copro, ['ville']) }}
                                    </div>
                                </div>
                    ...
                    @forelse ($syndicsAffiches as $syndic)
                        <div class="dr-record">
                            <div class="dr-record-header">
                                ...
                                <div class="dr-status success">
                                    <i class="fa-solid fa-id-card"></i> SIREN {{ dr_value($syndic, ['siren']) }}
                                </div>
                            </div>
                            <div class="dr-grid">
                                ...
                                <div class="dr-field">
                                    ...
                                    <div class="dr-field-value">
                                        {{ dr_value($syndic, ['chiffre_affaires']) }} / {{ dr_value($syndic, ['resultat']) }}
                                    </div>
                                </div>
                                <div class="dr-field">
                                    <div class="dr-field-label">Effectif</div>
