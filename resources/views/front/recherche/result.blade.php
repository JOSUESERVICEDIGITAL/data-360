...
<style>
    .result-page { background:#f8fafc; color:#1e293b; padding:2rem 0; }
    .result-container { max-width:1200px; margin:0 auto; padding:0 1.5rem; }
    .result-title { font-size:1.875rem; font-weight:700; margin-bottom:1.5rem; color:#0f172a; }
    .result-title span { color:#0053b3; }
    .dr-card { background:white; border-radius:16px; padding:1.5rem; margin-bottom:1.5rem; box-shadow:0 1px 3px rgba(0,0,0,.05); border:1px solid #e2e8f0; }
    .dr-card:hover { box-shadow:0 4px 12px rgba(0,0,0,.08); }
    .dr-card h2 { font-size:1.25rem; font-weight:600; margin-bottom:1rem; padding-bottom:.5rem; border-bottom:2px solid #0053b3; display:inline-block; color:#0053b3; }
    .dr-card h3 { font-size:1.125rem; font-weight:600; margin:1rem 0 .5rem; color:#1e293b; }
    .dr-badge { background:#e6f0ff; color:#0053b3; padding:.25rem .75rem; border-radius:20px; font-size:.75rem; font-weight:600; display:inline-block; margin-bottom:.75rem; }
    .dr-badge-success { background:#dcfce7; color:#166534; }
    .dr-badge-warning { background:#fff7ed; color:#9a3412; }
    .dr-info-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(210px,1fr)); gap:1rem; margin-top:.5rem; }
    .dr-info-item { background:#f8fafc; padding:.85rem; border-radius:12px; border:1px solid #eef2f7; }
    .dr-info-label { font-size:.72rem; text-transform:uppercase; letter-spacing:.5px; color:#64748b; margin-bottom:.25rem; }
    .dr-info-value { font-weight:700; color:#0f172a; word-break:break-word; }
    .dr-empty { color:#64748b; background:#f8fafc; border-radius:12px; padding:1rem; }
    .dr-hr { margin:1rem 0; border:none; border-top:1px solid #e2e8f0; }
    .dr-btn { display:inline-block; background:#0053b3; color:white; padding:.625rem 1.25rem; border-radius:8px; text-decoration:none; font-weight:600; transition:.2s ease; border:none; cursor:pointer; }
    .dr-btn:hover { background:#003d85; transform:translateY(-1px); }
    .dr-btn-white { background:white; color:#0053b3; }
    .dr-cta { background:linear-gradient(135deg,#0053b3 0%,#003d85 100%); color:white; text-align:center; }
    .dr-cta h3 { color:white; }
    .dr-cta p { margin-bottom:1rem; color:#eaf2ff; }
    details { margin-top:1rem; }
    summary { cursor:pointer; color:#0053b3; font-weight:700; }
    pre { white-space:pre-wrap; background:#f1f5f9; padding:15px; border-radius:8px; margin-top:10px; overflow:auto; font-size:12px; }
    @media (max-width:768px) {
        .result-container { padding:0 1rem; }
        .dr-card { padding:1rem; }
                ...
                <div class="dr-info-grid">
                    ...
                    <div class="dr-info-item">
                        ...
                        <div class="dr-info-value">
                            ...
                            <br>{{ dr_value($coproPrincipale, ['code_postal']) }} {{ dr_value($coproPrincipale, ['ville']) }}
                        </div>
                    </div>
                    ...
                    <div class="dr-info-item">
                        ...
                        <div class="dr-info-value">{{ dr_value($coproPrincipale, ['nom_copropriete', 'nom_usage_copropriete']) }}</div>
                    </div>
                ...
                @forelse(($resultat['batiments'] ?? []) as $batiment)
                    ...
                    <div class="dr-info-grid">
                        <div class="dr-info-item">
                            ...
                            <div class="dr-info-value">{{ dr_value($batiment, ['identifiant_bdnb', 'batiment_groupe_id', 'id_batiment_groupe', 'id']) }}</div>
                        </div>

                        <div class="dr-info-item">
                            <div class="dr-info-label">Type de bâtiment</div>
                            <div class="dr-info-value">{{ dr_value($batiment, ['type_batiment', 'usage_niveau_1_txt', 'usage_principal', 'usage_niveau_1']) }}</div>
                        </div>

                        <div class="dr-info-item">
                            <div class="dr-info-label">Année construction</div>
                            <div class="dr-info-value">{{ dr_value($batiment, ['annee_construction', 'annee_construction_estimee', 'annee_construction_dpe', 'annee_construction_max']) }}</div>
                        </div>

                        <div class="dr-info-item">
                            <div class="dr-info-label">Nombre logements</div>
                            <div class="dr-info-value">{{ dr_value($batiment, ['nombre_logements', 'nb_logements', 'nb_log', 'nombre_logement']) }}</div>
                        </div>

                        <div class="dr-info-item">
                            <div class="dr-info-label">Nombre niveaux</div>
                            <div class="dr-info-value">{{ dr_value($batiment, ['nombre_niveaux', 'nb_niveaux', 'nb_niveau', 'hauteur_nb_niveau']) }}</div>
                        </div>

                        <div class="dr-info-item">
                            <div class="dr-info-label">Hauteur</div>
                            <div class="dr-info-value">{{ dr_value($batiment, ['hauteur', 'hauteur_mean', 'hauteur_moyenne']) }}</div>
                        </div>

                        <div class="dr-info-item">
                            <div class="dr-info-label">Surface habitable</div>
                            <div class="dr-info-value">{{ dr_value($batiment, ['surface_habitable', 's_hab', 'surface_habitable_logement']) }}</div>
                        </div>

                        <div class="dr-info-item">
                            <div class="dr-info-label">Surface emprise sol</div>
                            <div class="dr-info-value">{{ dr_value($batiment, ['surface_emprise_sol', 's_emprise_sol', 'surface_emprise']) }}</div>
                        </div>

                        <div class="dr-info-item">
                            <div class="dr-info-label">DPE</div>
                            <div class="dr-info-value">{{ dr_value($batiment, ['classe_dpe', 'dpe_classe', 'classe_bilan_dpe']) }}</div>
                        </div>
                        ...
                        <div class="dr-info-item">
                            ...
                            <div class="dr-info-value">{{ dr_value($batiment, ['type_chauffage', 'chauffage', 'type_installation_chauffage']) }}</div>
                        </div>
            ...
            <div class="dr-card">
                ...
                @forelse(($resultat['coproprietes'] ?? []) as $copro)
                    ...
                    <div class="dr-info-grid">
                        <div class="dr-info-item">
                            <div class="dr-info-label">Nom résidence</div>
                            <div class="dr-info-value">{{ dr_value($copro, ['nom_copropriete', 'nom_usage_copropriete']) }}</div>
                        </div>
